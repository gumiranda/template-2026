...
async function seedFoodCategoriesHelper(ctx: MutationCtx) {
...
async function seedPromoBannersHelper(ctx: MutationCtx) {
...
export const seedFoodCategories = internalMutation({
  args: {},
  handler: async (ctx) => {
    await seedFoodCategoriesHelper(ctx);
  },
});
...
export const seedPromoBanners = internalMutation({
  args: {},
  handler: async (ctx) => {
    await seedPromoBannersHelper(ctx);
  },
});
